...
public class TimeConverter extends JFrame implements ActionListener, MouseListener, DocumentListener{
	...
	String[] textFieldOptions1 = {"Milliseconds", "Seconds", "Minutes", "Hours", "Days", "Weeks", "Years"};
	String[] textFieldOptions2 = {"Milliseconds", "Seconds", "Minutes", "Hours", "Days", "Weeks", "Years"};
	static double num1, num2, result;
	ImageIcon frameIcon = new ImageIcon("icon.png");
	JFrame timeConverterFrame = new JFrame("Time Converter");

	TimeConverter(){
		timeConverterFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		timeConverterFrame.setSize(335, 545);
		timeConverterFrame.setLocationRelativeTo(null);
		timeConverterFrame.setLayout(new BorderLayout());
		timeConverterFrame.getContentPane().setBackground(new Color(27, 27, 27));
		timeConverterFrame.setIconImage(frameIcon.getImage());
		...
		timeConverterFrame.add(topPanel, BorderLayout.NORTH);
		timeConverterFrame.add(botPanel, BorderLayout.SOUTH);
		
		option1 = new JComboBox<>(textFieldOptions1);
		option1.addActionListener(this);
		option1.setBounds(10, 100, 120, 20);

		option2 = new JComboBox<>(textFieldOptions1);
		...
		timeConverterFrame.setVisible(true);
	}
	...
	static int printMethod(double result) {
		if(result%1 == 0) {
			textField2.setText(String.format("%,d", (long)result)); // print in a whole number form and put comma every thousandths place
		}
		else {
			String numberAsString = Double.toString(result);
			if(numberAsString.contains(".")) {
				String decimalPart = numberAsString.split("\\.")[1];
				textField2.setText(String.format("%,." + decimalPart.length() + "f", result)); // print in a decimal form
